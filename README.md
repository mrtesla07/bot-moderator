...
- резервное копирование настроек в JSON и их перенос между чатами: /dfbackup и /dfrestore;
- два списка стоп-слов (мягкий/жёсткий) с независимыми действиями и лимитом предупреждений;
- быстрые админские твики: скрытие меню команд и очистка базы пользователей (/dfNoCommand, /dfCleaner, /dfCleanDeleted);
- расширенные отчёты: фильтры по правилам, рассылка админам и отдельный канал для premium;
- анкеты с авто-одобрением/отклонением и командами управления заявками;
...
- `/dfbackup` — выгрузить JSON-файл с настройками чата для переноса.
- `/dfrestore` — восстановить настройки из JSON-файла или текста (ответом на команду).
- `/addstopword [номер] <слово>` — добавить стоп-слово в выбранный список (по умолчанию мягкий).
- `/delstopword [номер] <слово>` — удалить стоп-слово из заданного списка или из обоих, если номер не указан.
- `/liststopwords` — показать оба списка со статистикой и текущим лимитом предупреждений.
- `/setstoplimit <1-10>` — задать порог предупреждений для реакции на стоп-слова.
- `/dfnocommand` — скрыть меню команд в чате и вернуть его при повторном вызове.
- `/dfcleaner` — удалить записи пользователей, покинувших чат.
- `/dfcleandeleted` — удалить записи удалённых аккаунтов.
- `/dfrequests` — показать активные заявки и ответы анкеты.
- `/dfapprove <user_id>` — одобрить заявку пользователя.
- `/dfreject <user_id> [причина]` — отклонить заявку с опциональным комментарием.
...
- Поле `settings` у чата — JSON-конфигурация модулей, изменения применяются сразу после сохранения.
